  ...
  return (
    <div className="h-full flex flex-col">
      <div className="p-6 pb-4 text-center">
        <div className="flex items-center justify-center gap-3 mb-3">
          <div className="p-2 bg-gradient-to-br from-[#78BE20]/20 to-[#4ade80]/20 rounded-full">
            <Crown className="w-5 h-5 text-[#78BE20]" />
          </div>
      ...
      <div className="flex-1 px-6 pb-6 overflow-y-auto">
        <div className={`grid gap-3 ${isSinglePlayer ? 'grid-cols-1' : 'grid-cols-1'}`}>
          {Object.values(playerStats).map((playerStat) => (
            <div 
              key={playerStat.player}
              className="bg-gradient-to-br from-[#1e2129] to-[#141923] rounded-lg shadow-md hover:shadow-lg transition-all duration-300 border border-gray-700/30 hover:border-[#78BE20]/30 flex flex-col overflow-hidden"
            >
              <div className="p-4 space-y-3 flex-grow">
                <div className="flex items-center gap-3">
                  <div className="relative group flex-shrink-0">
                    {playerStat.image_url ? (
                      <img
                        ...
                        className="w-12 h-12 rounded-full object-cover border-2 border-[#78BE20] bg-[#1e2129] group-hover:border-[#4ade80] transition-colors duration-300"
                        onError={(e) => {
                      ...
                      <div className="w-12 h-12 rounded-full border-2 border-[#78BE20] bg-[#1e2129] flex items-center justify-center group-hover:border-[#4ade80] transition-colors duration-300">
                  ...
                  <div className="min-w-0 flex-grow">
                    <h3 className="text-lg font-bold text-white truncate group-hover:text-[#78BE20] transition-colors duration-300">
                      {playerStat.player}
                    </h3>
                ...
                <div className="space-y-1 max-h-64 overflow-y-auto">
                    ...
                    .map((stat, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-right flex-shrink-0 ml-2 relative pr-6">
                  ...
                  {playerStat.stats.length > 5 && (
                    <button
                      ...
                      className="w-full mt-2 py-2 px-3 text-sm font-medium text-[#78BE20] hover:text-[#4ade80] transition-colors duration-300 flex items-center justify-center gap-2"
                    >
                      {expandedPlayers[playerStat.player] ? (
                        <>Show Less <ChevronUp className="w-4 h-4" /></>
                      ) : (
                        <>Show {playerStat.stats.length - 5} More <ChevronDown className="w-4 h-4" /></>
                      )}
                    </button>
                  )}
                </div>
              ...
              <div className="px-4 py-3 border-t border-gray-700/50 bg-gradient-to-r from-[#0f1119] to-[#141923] rounded-b-lg">
